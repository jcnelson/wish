...
int main(int argc, char *argv[])
{
   ...
   int len = strlen("http://:/SETENV/=") + strlen(wishHost) + strlen(portNumber) + strlen(argv[1]) + strlen(argv[2]) + 2;

   char * url = (char *) calloc(len + 1, 1);

   sprintf(url, "http://%s:%s/SETENV/%s=%s", wishHost, portNumber, argv[1], argv[2]);
   ...
   if(res != 0 || resp.status != 200)
      printf("Error: gsetenv didn't work! rc = %d resp.status = %d\n", res, resp.status);
